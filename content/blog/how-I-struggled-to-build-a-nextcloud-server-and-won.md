...
Also, I should note - no one on the internet really cared to explain how a RAID setup would work with Nextcloud. Eventually I figured this out - the RAID setup is seen by the OS as one drive, so just politely ask Nextcloud to use that instead of the SD card. I ended up having to [create a symlink from Nextcloud's data directory to a directory on my RAID device](https://docs.nextcloud.com/server/latest/admin_manual/issues/general_troubleshooting.html#troubleshooting-data-directory), but this wasn't the worst thing in the world.
